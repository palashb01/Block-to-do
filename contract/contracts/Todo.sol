...
contract Todo {
    ...
    function createTask(string calldata _content) public {
        ...
        addressToTasks[msg.sender].push(taskId);
        taskToAddress[taskId] = msg.sender;

        taskId++;

        emit TaskAdded(taskId, msg.sender, _content);
    }

    function toggleTask(uint256 _taskId) public {
        tasks[_taskId].completed = !tasks[_taskId].completed;

        emit TaskToggled(_taskId, msg.sender, tasks[_taskId].completed);
    }
    ...
    function setTaskStatus(uint256 _taskId, bool _status) public {
        ...
        emit TaskToggled(_taskId, msg.sender, _status);
    }
    ...
    function completeTask(uint256 _taskId) public {
        ...
        emit TaskToggled(_taskId, msg.sender, true);
    }
